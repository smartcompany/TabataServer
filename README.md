...
| GET | `/api/dashboard/ai-usage` | 관리자: AI 루틴 토큰·예상 비용 |
| POST | `/api/analytics/events` | 앱: 익명 사용자 여정 이벤트 batch 수집 |
| GET | `/api/dashboard/product-analytics` | 관리자: 퍼널·기능 사용·익명 사용자 여정 |
...
사용자 여정 이벤트는 `tabata_product_events`에 저장합니다. 앱 배포 전에
Supabase SQL Editor에서 최신 `server/supabase/schema.sql` 전체를 다시
실행해야 하며, idempotent 스키마라 기존 데이터는 유지됩니다.
